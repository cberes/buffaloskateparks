+++
title = 'Greece'
latitude = 43.2598637
longitude = -77.700348
#https://13wham.com/news/local/greece-celebrates-grand-opening-of-eco-friendly-skate-park-at-town-hall
date = 2025-05-29T12:00:00-04:00
featured_image = 'images/01.jpg'
categories = ['outdoor', 'new york']
tags = ['greece','greater-rochester','outdoor','concrete','box','manual-pad','flat-rail','down-rail','hip','stairs','bowl','3-ft-transition','4-ft-transition','5-ft-transition']
+++

{{< photos >}}

## Location

Greece Town Hall, 3 Vince Tofany Blvd, Rochester, NY 14612

## Description

Large concrete skatepark with an ancient-Greece theme in the Rochester suburbs.
It's behind the town hall (past the tennis courts).
Greece has a great library&mdash;be sure to stop by.

### Bathrooms

There are bathrooms in the senior center and the library.

## Features

- bowl
- 3' quarter pipe (no coping)
- 4' quarter pipes with 5' extension, 6' bank wall
- 4' miniramp
- 5' quarter pipe
- boxes
- manual pad
- flat rail
- down rail
- rainbow rail
- pyramid
- small bump, large bump
- 4-stair with hand rail
- slappy curb
- euro gap
- pump track
